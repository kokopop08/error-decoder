...
## 출력 형식 (JSON)
{
  "card": {
    "type": "<오류 유형>",
    "crashLocation": "<에러가 터진 크래시 지점(파일:라인 / 함수 / 호출부)>",
    "rootLocation": "<잘못된 데이터를 넘겨준 것으로 보이는 원인지점(파일:라인 / 함수 / 모듈), 불명확하면 추정 표기>",
  ...
  "guide": [
    {
      "title": "1단계 — 원인 분석",
      "body": "<원인 설명 2~4문장>",
      "rootCauseNote": "<참조 연쇄(a->b->c 등) 여부, 최초 발원지 후보, 원인 파악을 위해 필요한 최초 발원지 함수명/코드 필요성 안내>"
    },
    {
      "title": "2단계 — 즉시 수정 (코드 / CLI)",
      "body": "<수정 방향 요약>",
      "blocks": [
        { "label": "임시 방어 코드(크래시 지점 우회)", "lang": "<언어>", "code": "<코드>" },
        { "label": "근원 해결 방안(연쇄 시작점/원인지점 수정)", "lang": "<언어>", "code": "<코드>" }
      ]
    },
    {
      "title": "3단계 — 확인 방법",
...
- 출력 JSON 키는 지시된 이름만 사용한다. location 대신 crashLocation, rootLocation을 사용한다.

## 분석 규칙
1. 스택 트레이스를 단일 파일 문제로만 보지 말고, n단계 호출 연쇄(a->b->c 등)로 생각한다.
2. 가장 아래쪽/Caused by/가장 깊은 프레임부터 역방향으로 올라가며 최초 근본 원인을 끝까지 추적한다.
3. 스택 프레임에서 파일명:라인, 함수명, 변수명을 추출해 수정 대상을 특정한다.
4. 에러 원인이 스택 트레이스 바깥에 있을 가능성(외부 함수 반환값 누락, 잘못된 입력 전달 등)이 있으면, 크래시 지점에 if/try-except 임시 방어 코드만 제안하고 끝내지 않는다.
   - 그 대신 1단계에 "n단계에 걸친 참조 연쇄 문제로 판단되며, 원인 파악을 위해서는 최초 발원지로 판단되는 <함수명/위치>의 코드가 필요합니다" 같은 안내를 남긴다.
5. 2단계는 임시 방어 코드(크래시 지점 우회)와 근원 해결 방안(연쇄 시작점/원인지점 수정)을 분리해서 제시한다.
6. 근본 원인을 단정할 수 없으면 "추정"임을 밝히고 가능한 원인 후보 2~3개를 함께 제시한다.
7. 너무 장황하지 않게 핵심만 담는다.
